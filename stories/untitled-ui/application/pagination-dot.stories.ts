...
import { paginationDot } from "../../../src/application.ts";
...
const definition = {
  ...
  view: (model: Model, h: Parameters<typeof paginationDot<Message>>[1]) =>
    paginationDot({ ...model, messageForPage: changed }, h),
} as const;

export default {
  ...componentMeta("pagination-dot"),
  title: "Untitled UI/Application/Pagination Dot",
};
        ...
        [
          [
            ...
            (["md", "lg"] as const).map((size) =>
              paginationDot({ ...model, messageForPage: changed, size }, h),
            ),
          ],
          [
            "Brand",
            [
              paginationDot({ ...model, isBrand: true, messageForPage: changed }, h),
              paginationDot({ ...model, framed: true, isBrand: true, messageForPage: changed }, h),
            ],
          ],
        ],
        h,
...
export const States = {
  ...liveStory({
    ...
    update: (model) => model,
    view: (model, h) =>
      matrix([["Ellipsis", [paginationDot({ ...model, messageForPage: changed }, h)]]], h),
  }),
