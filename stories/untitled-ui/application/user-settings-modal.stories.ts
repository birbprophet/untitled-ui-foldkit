...
import { userSettingsModal } from "ui/application";
import type { UserSettingsCountry, UserSettingsField } from "ui/application";
...
const makeDefinition = (state: "inactive" | "partial", initiallyOpen = true) => ({
  ...
  init: (args: typeof Args.Type) => {
    ...
    return [
      {
        email: edited ? "maya.chen@example.com" : "hi@siennahewitt.com",
...
export const Interactions = {
  ...
  play: async ({ canvasElement }: { canvasElement: HTMLElement }) => {
    ...
    await expect(dialog.querySelector("#input-email")).toHaveValue("maya.chen@example.com");
