...
import { dropdownAccountCardXS } from "../../../src/base.ts";
...
type Message =
  ...
  | Readonly<{ _tag: "Closed" | "SignOutToggled" | "ThemeToggled" | "Toggled" }>;

const specimen = (model: Model, h: Parameters<typeof dropdownAccountCardXS<Message>>[1]) =>
...
const definition = (initiallyOpen: boolean) => ({
  ...
  update: (model: Model, message: Message): Model => {
    if (message._tag === "Toggled") {
      return { ...model, isOpen: !model.isOpen, isSignOutOpen: false };
    }
    if (message._tag === "Closed") {
      return { ...model, isOpen: false, isSignOutOpen: false };
    }
    ...
    if (message._tag === "SignOutToggled") {
      return { ...model, isSignOutOpen: !model.isSignOutOpen };
    }
  ...
  view: (model: Model, h: Parameters<typeof dropdownAccountCardXS<Message>>[1]) =>
    h.div([h.Class("min-h-[36rem]")], [specimen(model, h)]),
});

export default {
  ...componentMeta("dropdown-account-card-xs"),
  title: "Untitled UI/Base/Dropdown Account Card XS",
};
