...
import { heroSplitForm01 } from "../../../../../packages/ui/src/marketing/hero-split-form-01.ts";
...
const Primary = m("HeroSplitForm01Primary");
const EmailInput = m("HeroSplitForm01EmailInput", { email: S.String });
const Submit = m("HeroSplitForm01Submit");
const Secondary = m("HeroSplitForm01Secondary");
const Navigated = m("HeroSplitForm01Navigated", { href: S.String });
const MobileOpened = m("HeroSplitForm01MobileOpened");
const MobileClosed = m("HeroSplitForm01MobileClosed");
const AccountPressed = m("HeroSplitForm01AccountPressed");
const SearchChanged = m("HeroSplitForm01SearchChanged", { value: S.String });
type Message =
...
const definition = {
  ...
  update: (model: Model, message: Message): Model => {
    if (message._tag === "HeroSplitForm01MobileOpened") {
      return { ...model, isMobileOpen: true };
    }
    if (message._tag === "HeroSplitForm01MobileClosed") {
      return { ...model, isMobileOpen: false };
    }
    if (message._tag === "HeroSplitForm01SearchChanged") {
      return { ...model, searchValue: message.value };
    }
    return model;
  },
  view: (model: Model, h: Parameters<typeof heroSplitForm01<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [
        heroSplitForm01(
...
export default {
  ...componentMeta("hero-split-form-01"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Hero Split Form 01",
};
