...
import { heroAbstractAngles04 } from "../../../../../packages/ui/src/marketing/hero-abstract-angles-04.ts";
...
const Primary = m("HeroAbstractAngles04Primary");
const Secondary = m("HeroAbstractAngles04Secondary");
const Navigated = m("HeroAbstractAngles04Navigated", { href: S.String });
const MobileOpened = m("HeroAbstractAngles04MobileOpened");
const MobileClosed = m("HeroAbstractAngles04MobileClosed");
const AccountPressed = m("HeroAbstractAngles04AccountPressed");
const SearchChanged = m("HeroAbstractAngles04SearchChanged", { value: S.String });
type Message =
  | typeof Primary.Type
...
const definition = {
  ...
  update: (model: Model, message: Message): Model => {
    if (message._tag === "HeroAbstractAngles04MobileOpened") {
      return { ...model, isMobileOpen: true };
    }
    if (message._tag === "HeroAbstractAngles04MobileClosed") {
      return { ...model, isMobileOpen: false };
    }
    if (message._tag === "HeroAbstractAngles04SearchChanged") {
      return { ...model, searchValue: message.value };
    }
    return model;
  },
  view: (model: Model, h: Parameters<typeof heroAbstractAngles04<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [
        heroAbstractAngles04(
          {
            ...model,
...
export default {
  ...componentMeta("hero-abstract-angles-04"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Hero Abstract Angles 04",
};
