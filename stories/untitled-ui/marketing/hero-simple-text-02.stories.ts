...
import { heroSimpleText02 } from "../../../../../packages/ui/src/marketing/hero-simple-text-02.ts";
...
const Primary = m("HeroSimpleText02Primary");
const Secondary = m("HeroSimpleText02Secondary");
const Navigated = m("HeroSimpleText02Navigated", { href: S.String });
const MobileOpened = m("HeroSimpleText02MobileOpened");
const MobileClosed = m("HeroSimpleText02MobileClosed");
const AccountPressed = m("HeroSimpleText02AccountPressed");
const SearchChanged = m("HeroSimpleText02SearchChanged", { value: S.String });
type Message =
  | typeof Primary.Type
...
const definition = {
  ...
  update: (model: Model, message: Message): Model => {
    if (message._tag === "HeroSimpleText02MobileOpened") {
      return { ...model, isMobileOpen: true };
    }
    if (message._tag === "HeroSimpleText02MobileClosed") {
      return { ...model, isMobileOpen: false };
    }
    if (message._tag === "HeroSimpleText02SearchChanged") {
      return { ...model, searchValue: message.value };
    }
    return model;
  },
  view: (model: Model, h: Parameters<typeof heroSimpleText02<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [
        heroSimpleText02(
...
export default {
  ...componentMeta("hero-simple-text-02"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Hero Simple Text 02",
};
