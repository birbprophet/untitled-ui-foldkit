...
import { heroSplitForm02 } from "../../../../../packages/ui/src/marketing/hero-split-form-02.ts";
...
const Primary = m("HeroSplitForm02Primary");
const EmailInput = m("HeroSplitForm02EmailInput", { email: S.String });
const Submit = m("HeroSplitForm02Submit");
const Secondary = m("HeroSplitForm02Secondary");
const Navigated = m("HeroSplitForm02Navigated", { href: S.String });
const MobileOpened = m("HeroSplitForm02MobileOpened");
const MobileClosed = m("HeroSplitForm02MobileClosed");
const AccountPressed = m("HeroSplitForm02AccountPressed");
const SearchChanged = m("HeroSplitForm02SearchChanged", { value: S.String });
type Message =
...
const definition = {
  ...
  update: (model: Model, message: Message): Model => {
    if (message._tag === "HeroSplitForm02MobileOpened") {
      return { ...model, isMobileOpen: true };
    }
    if (message._tag === "HeroSplitForm02MobileClosed") {
      return { ...model, isMobileOpen: false };
    }
    if (message._tag === "HeroSplitForm02SearchChanged") {
      return { ...model, searchValue: message.value };
    }
    return model;
  },
  view: (model: Model, h: Parameters<typeof heroSplitForm02<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [
        heroSplitForm02(
...
export default {
  ...componentMeta("hero-split-form-02"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Hero Split Form 02",
};
