...
import { bannerSingleActionDefault } from "../../../../../packages/ui/src/marketing/banner-single-action-default.ts";
...
const Action = m("BannerSingleActionDefaultAction");
const Dismissed = m("BannerSingleActionDefaultDismissed");
...
const definition = {
  ...
  update: (model: Model, message: Message): Model =>
    message._tag === "BannerSingleActionDefaultDismissed"
      ? { ...model, isVisible: false }
      : { ...model, actionHref: "#update-opened" },
  view: (model: Model, h: Parameters<typeof bannerSingleActionDefault<Message>>[1]) =>
    model.isVisible
      ? bannerSingleActionDefault({ ...model, onAction: Action(), onDismiss: Dismissed() }, h)
...
export default {
  ...componentMeta("banner-single-action-default"),
  title: "Untitled UI/Marketing/Banners/Banner Single Action Default",
};
