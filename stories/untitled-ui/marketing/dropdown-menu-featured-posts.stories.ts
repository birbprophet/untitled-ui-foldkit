...
import { dropdownMenuFeaturedPosts } from "../../../../../packages/ui/src/marketing/dropdown-menu-featured-posts.ts";
...
const definition = {
  ...
  view: (model: Model, h: Parameters<typeof dropdownMenuFeaturedPosts<Message>>[1]) =>
    h.div([h.Class("-m-8")], [dropdownMenuFeaturedPosts({ ...model, onAllPosts: Action({ id: "all-posts" }), onItem: (itemId) => Action({ id: itemId }), onPost: (postId) => Action({ id: postId }) }, h)]),
} as const;
...
const args = {
  ...
  posts: [
    { id: "auto-layout", imageAlt: "Auto Layout explained", imageSrc: "https://www.untitledui.com/marketing/auto-layout.webp", subtitle: "Jump right in—get an overview of the basics.", title: "Auto Layout explained" },
  ],
