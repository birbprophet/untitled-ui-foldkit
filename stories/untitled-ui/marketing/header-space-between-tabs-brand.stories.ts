...
import { headerSpaceBetweenTabsBrand } from "../../../../../packages/ui/src/marketing/header-space-between-tabs-brand.ts";
...
const TabSelected = m("HeaderSpaceBetweenTabsBrandTabSelected", { tabId: S.String });
...
const definition = {
  ...
  update: (model: Model, message: Message): Model =>
    message._tag === "HeaderSpaceBetweenTabsBrandTabSelected"
      ? { ...model, selectedTabId: message.tabId }
      : model,
  view: (model: Model, h: Parameters<typeof headerSpaceBetweenTabsBrand<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [headerSpaceBetweenTabsBrand({ ...model, onTab: (tabId) => TabSelected({ tabId }) }, h)],
    ),
} as const;
...
export default {
  ...componentMeta("header-space-between-tabs-brand"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Header Space Between Tabs Brand",
};
