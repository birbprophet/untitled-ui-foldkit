...
import { headerCenteredTabsBrand } from "../../../../../packages/ui/src/marketing/header-centered-tabs-brand.ts";
...
const TabSelected = m("HeaderCenteredTabsBrandTabSelected", { tabId: S.String });
...
const definition = {
  ...
  update: (model: Model, message: Message): Model =>
    message._tag === "HeaderCenteredTabsBrandTabSelected"
      ? { ...model, selectedTabId: message.tabId }
      : model,
  view: (model: Model, h: Parameters<typeof headerCenteredTabsBrand<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [headerCenteredTabsBrand({ ...model, onTab: (tabId) => TabSelected({ tabId }) }, h)],
    ),
} as const;
...
export default {
  ...componentMeta("header-centered-tabs-brand"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Header Centered Tabs Brand",
};
