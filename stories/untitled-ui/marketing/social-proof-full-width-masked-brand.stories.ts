...
const section = (args: SectionArgs, h: HtmlBuilder<{ readonly _tag: "Noop" }>) =>
  h.div([h.Class("-m-8")], [socialProofFullWidthMaskedBrand(args, h)]);
...
const args = {
  ...
  description: "See why thousands of product teams run their roadmap on Siglata.",
  eyebrow: "Customers",
...
export const Interactions = {
  ...
  play: async ({ canvasElement }: { readonly canvasElement: HTMLElement }) => {
    ...
    await expect(canvas.getByRole("heading")).toHaveTextContent("Trusted by teams worldwide");
  },
};
