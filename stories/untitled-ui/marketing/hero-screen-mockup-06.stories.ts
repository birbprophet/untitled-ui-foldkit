...
const Primary = m("HeroScreenMockup06Primary");
const Secondary = m("HeroScreenMockup06Secondary");
const Navigated = m("HeroScreenMockup06Navigated", { href: S.String });
const MobileOpened = m("HeroScreenMockup06MobileOpened");
...
const definition = {
  ...
  update: (model: Model, message: Message): Model => {
    if (message._tag === "HeroScreenMockup06MobileOpened") {
      return { ...model, isMobileOpen: true };
    }
    if (message._tag === "HeroScreenMockup06MobileClosed") {
      return { ...model, isMobileOpen: false };
    }
    if (message._tag === "HeroScreenMockup06SearchChanged") {
      return { ...model, searchValue: message.value };
    }
    return model;
  },
  view: (model: Model, h: Parameters<typeof heroScreenMockup06<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [
        heroScreenMockup06(
...
export default {
  ...componentMeta("hero-screen-mockup-06"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Hero Screen Mockup 06",
};
