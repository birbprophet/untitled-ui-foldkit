...
import { heroAbstractAngles03 } from "../../../src/marketing/hero-abstract-angles-03.ts";
...
const Primary = m("HeroAbstractAngles03Primary");
const Secondary = m("HeroAbstractAngles03Secondary");
const Navigated = m("HeroAbstractAngles03Navigated", { href: S.String });
const MobileOpened = m("HeroAbstractAngles03MobileOpened");
const MobileClosed = m("HeroAbstractAngles03MobileClosed");
const AccountPressed = m("HeroAbstractAngles03AccountPressed");
const SearchChanged = m("HeroAbstractAngles03SearchChanged", { value: S.String });
const Badge = m("HeroAbstractAngles03Badge");
type Message =
  | typeof Primary.Type
...
const definition = {
  ...
  update: (model: Model, message: Message): Model => {
    if (message._tag === "HeroAbstractAngles03MobileOpened") {
      return { ...model, isMobileOpen: true };
    }
    if (message._tag === "HeroAbstractAngles03MobileClosed") {
      return { ...model, isMobileOpen: false };
    }
    if (message._tag === "HeroAbstractAngles03SearchChanged") {
      return { ...model, searchValue: message.value };
    }
    return model;
  },
  view: (model: Model, h: Parameters<typeof heroAbstractAngles03<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [
        heroAbstractAngles03(
          {
            ...model,
...
export default {
  ...componentMeta("hero-abstract-angles-03"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Hero Abstract Angles 03",
};
