...
import { headerSpaceBetweenButtons } from "../../../../../packages/ui/src/marketing/header-space-between-buttons.ts";
...
const Primary = m("HeaderSpaceBetweenButtonsPrimary");
const Secondary = m("HeaderSpaceBetweenButtonsSecondary");
...
const definition = {
  ...
  view: (model: Model, h: Parameters<typeof headerSpaceBetweenButtons<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [headerSpaceBetweenButtons({ ...model, onPrimary: Primary(), onSecondary: Secondary() }, h)],
    ),
} as const;
...
export default {
  ...componentMeta("header-space-between-buttons"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Header Space Between Buttons",
};
