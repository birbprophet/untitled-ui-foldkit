...
import { headerLeftEmailBrand } from "../../../../../packages/ui/src/marketing/header-left-email-brand.ts";
...
const EmailInput = m("HeaderLeftEmailBrandEmailInput", { email: S.String });
const Submit = m("HeaderLeftEmailBrandSubmit");
...
const definition = {
  ...
  update: (model: Model, message: Message): Model =>
    message._tag === "HeaderLeftEmailBrandEmailInput" ? { ...model, email: message.email } : model,
  view: (model: Model, h: Parameters<typeof headerLeftEmailBrand<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [
        headerLeftEmailBrand(
...
export default {
  ...componentMeta("header-left-email-brand"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Header Left Email Brand",
};
