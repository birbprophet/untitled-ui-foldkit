...
import { dropdownMenuSimple } from "../../../../../packages/ui/src/marketing/dropdown-menu-simple.ts";
...
const definition = {
  ...
  view: (_model: Model, h: Parameters<typeof dropdownMenuSimple<Message>>[1]) =>
    h.div([h.Class("-m-8")], [
      dropdownMenuSimple({ items: [...marketingMenuItems], onItem: (id) => ItemSelected({ id }) }, h),
    ]),
} as const;
const args = {} as const;
export default { ...componentMeta("dropdown-menu-simple"), title: "Untitled UI/Marketing/Header Navigation/Dropdown Menu Simple" };
...
export const Interactions = { ...liveStory(definition), args, play: async ({ canvasElement }: { readonly canvasElement: HTMLElement }) => { await waitForStoryReady(canvasElement); await expect(within(canvasElement).getAllByRole("link").length).toBeGreaterThan(0); } };
