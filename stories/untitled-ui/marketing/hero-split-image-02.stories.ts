...
import { heroSplitImage02 } from "../../../../../packages/ui/src/marketing/hero-split-image-02.ts";
...
const Primary = m("HeroSplitImage02Primary");
const Secondary = m("HeroSplitImage02Secondary");
const Navigated = m("HeroSplitImage02Navigated", { href: S.String });
const MobileOpened = m("HeroSplitImage02MobileOpened");
const MobileClosed = m("HeroSplitImage02MobileClosed");
const AccountPressed = m("HeroSplitImage02AccountPressed");
const SearchChanged = m("HeroSplitImage02SearchChanged", { value: S.String });
type Message =
  | typeof Primary.Type
...
const definition = {
  ...
  update: (model: Model, message: Message): Model => {
    if (message._tag === "HeroSplitImage02MobileOpened") {
      return { ...model, isMobileOpen: true };
    }
    if (message._tag === "HeroSplitImage02MobileClosed") {
      return { ...model, isMobileOpen: false };
    }
    if (message._tag === "HeroSplitImage02SearchChanged") {
      return { ...model, searchValue: message.value };
    }
    return model;
  },
  view: (model: Model, h: Parameters<typeof heroSplitImage02<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [
        heroSplitImage02(
...
export default {
  ...componentMeta("hero-split-image-02"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Hero Split Image 02",
};
