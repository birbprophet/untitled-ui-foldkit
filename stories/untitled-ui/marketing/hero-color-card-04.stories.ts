...
import { heroColorCard04 } from "../../../../../packages/ui/src/marketing/hero-color-card-04.ts";
...
const Primary = m("HeroColorCard04Primary");
const EmailInput = m("HeroColorCard04EmailInput", { email: S.String });
const Submit = m("HeroColorCard04Submit");
const Secondary = m("HeroColorCard04Secondary");
const Navigated = m("HeroColorCard04Navigated", { href: S.String });
const MobileOpened = m("HeroColorCard04MobileOpened");
const MobileClosed = m("HeroColorCard04MobileClosed");
const AccountPressed = m("HeroColorCard04AccountPressed");
const SearchChanged = m("HeroColorCard04SearchChanged", { value: S.String });
type Message =
...
const definition = {
  ...
  update: (model: Model, message: Message): Model => {
    if (message._tag === "HeroColorCard04MobileOpened") {
      return { ...model, isMobileOpen: true };
    }
    if (message._tag === "HeroColorCard04MobileClosed") {
      return { ...model, isMobileOpen: false };
    }
    if (message._tag === "HeroColorCard04SearchChanged") {
      return { ...model, searchValue: message.value };
    }
    return model;
  },
  view: (model: Model, h: Parameters<typeof heroColorCard04<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [
        heroColorCard04(
...
export default {
  ...componentMeta("hero-color-card-04"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Hero Color Card 04",
};
