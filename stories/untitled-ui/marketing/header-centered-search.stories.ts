...
import { headerCenteredSearch } from "../../../../../packages/ui/src/marketing/header-centered-search.ts";
...
const SearchInput = m("HeaderCenteredSearchSearchInput", { value: S.String });
...
const definition = {
  ...
  update: (model: Model, message: Message): Model =>
    message._tag === "HeaderCenteredSearchSearchInput"
      ? { ...model, searchValue: message.value }
      : model,
  view: (model: Model, h: Parameters<typeof headerCenteredSearch<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [headerCenteredSearch({ ...model, onSearch: (value) => SearchInput({ value }) }, h)],
    ),
} as const;
...
export default {
  ...componentMeta("header-centered-search"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Header Centered Search",
};
