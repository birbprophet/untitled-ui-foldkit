...
const Allowed = m("BannerDualActionBrandAllowed");
const Declined = m("BannerDualActionBrandDeclined");
const Dismissed = m("BannerDualActionBrandDismissed");
...
const definition = {
  ...
  update: (model: Model, message: Message): Model => {
    if (message._tag === "BannerDualActionBrandDismissed") {
      return { ...model, isVisible: false };
    }
    if (message._tag === "BannerDualActionBrandAllowed") {
      return { ...model, allowLabel: "Allowed", choice: "allowed" };
    }
    return { ...model, choice: "declined", declineLabel: "Declined" };
  },
  view: (model: Model, h: Parameters<typeof bannerDualActionBrand<Message>>[1]) =>
    model.isVisible
...
export default {
  ...componentMeta("banner-dual-action-brand"),
  title: "Untitled UI/Marketing/Banners/Banner Dual Action Brand",
};
