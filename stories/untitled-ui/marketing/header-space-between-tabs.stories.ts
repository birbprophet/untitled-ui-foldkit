...
const TabSelected = m("HeaderSpaceBetweenTabsTabSelected", { tabId: S.String });
...
const definition = {
  ...
  update: (model: Model, message: Message): Model =>
    message._tag === "HeaderSpaceBetweenTabsTabSelected"
      ? { ...model, selectedTabId: message.tabId }
      : model,
  view: (model: Model, h: Parameters<typeof headerSpaceBetweenTabs<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [headerSpaceBetweenTabs({ ...model, onTab: (tabId) => TabSelected({ tabId }) }, h)],
    ),
} as const;
...
export default {
  ...componentMeta("header-space-between-tabs"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Header Space Between Tabs",
};
