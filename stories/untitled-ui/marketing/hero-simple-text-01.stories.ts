...
import { heroSimpleText01 } from "../../../../../packages/ui/src/marketing/hero-simple-text-01.ts";
...
const Primary = m("HeroSimpleText01Primary");
const Secondary = m("HeroSimpleText01Secondary");
const Navigated = m("HeroSimpleText01Navigated", { href: S.String });
const MobileOpened = m("HeroSimpleText01MobileOpened");
const MobileClosed = m("HeroSimpleText01MobileClosed");
const AccountPressed = m("HeroSimpleText01AccountPressed");
const SearchChanged = m("HeroSimpleText01SearchChanged", { value: S.String });
type Message =
  | typeof Primary.Type
...
const definition = {
  ...
  update: (model: Model, message: Message): Model => {
    if (message._tag === "HeroSimpleText01MobileOpened") {
      return { ...model, isMobileOpen: true };
    }
    if (message._tag === "HeroSimpleText01MobileClosed") {
      return { ...model, isMobileOpen: false };
    }
    if (message._tag === "HeroSimpleText01SearchChanged") {
      return { ...model, searchValue: message.value };
    }
    return model;
  },
  view: (model: Model, h: Parameters<typeof heroSimpleText01<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [
        heroSimpleText01(
...
export default {
  ...componentMeta("hero-simple-text-01"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Hero Simple Text 01",
};
