...
import { heroColorCard02 } from "../../../../../packages/ui/src/marketing/hero-color-card-02.ts";
...
const Primary = m("HeroColorCard02Primary");
const EmailInput = m("HeroColorCard02EmailInput", { email: S.String });
const Submit = m("HeroColorCard02Submit");
const Secondary = m("HeroColorCard02Secondary");
const Navigated = m("HeroColorCard02Navigated", { href: S.String });
const MobileOpened = m("HeroColorCard02MobileOpened");
const MobileClosed = m("HeroColorCard02MobileClosed");
const AccountPressed = m("HeroColorCard02AccountPressed");
const SearchChanged = m("HeroColorCard02SearchChanged", { value: S.String });
type Message =
...
const definition = {
  ...
  update: (model: Model, message: Message): Model => {
    if (message._tag === "HeroColorCard02MobileOpened") {
      return { ...model, isMobileOpen: true };
    }
    if (message._tag === "HeroColorCard02MobileClosed") {
      return { ...model, isMobileOpen: false };
    }
    if (message._tag === "HeroColorCard02SearchChanged") {
      return { ...model, searchValue: message.value };
    }
    return model;
  },
  view: (model: Model, h: Parameters<typeof heroColorCard02<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [
        heroColorCard02(
...
export default {
  ...componentMeta("hero-color-card-02"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Hero Color Card 02",
};
