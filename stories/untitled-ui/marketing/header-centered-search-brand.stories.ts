...
import { headerCenteredSearchBrand } from "../../../../../packages/ui/src/marketing/header-centered-search-brand.ts";
...
const SearchInput = m("HeaderCenteredSearchBrandSearchInput", { value: S.String });
...
const definition = {
  ...
  update: (model: Model, message: Message): Model =>
    message._tag === "HeaderCenteredSearchBrandSearchInput"
      ? { ...model, searchValue: message.value }
      : model,
  view: (model: Model, h: Parameters<typeof headerCenteredSearchBrand<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [headerCenteredSearchBrand({ ...model, onSearch: (value) => SearchInput({ value }) }, h)],
    ),
} as const;
...
export default {
  ...componentMeta("header-centered-search-brand"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Header Centered Search Brand",
};
