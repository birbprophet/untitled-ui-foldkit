...
import { bannerSingleActionBrand } from "../../../../../packages/ui/src/marketing/banner-single-action-brand.ts";
...
const Action = m("BannerSingleActionBrandAction");
const Dismissed = m("BannerSingleActionBrandDismissed");
...
const definition = {
  ...
  update: (model: Model, message: Message): Model =>
    message._tag === "BannerSingleActionBrandDismissed"
      ? { ...model, isVisible: false }
      : { ...model, actionHref: "#update-opened" },
  view: (model: Model, h: Parameters<typeof bannerSingleActionBrand<Message>>[1]) =>
    model.isVisible
      ? bannerSingleActionBrand({ ...model, onAction: Action(), onDismiss: Dismissed() }, h)
...
export default {
  ...componentMeta("banner-single-action-brand"),
  title: "Untitled UI/Marketing/Banners/Banner Single Action Brand",
};
