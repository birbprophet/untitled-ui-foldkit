...
import { heroSplitImage06 } from "../../../../../packages/ui/src/marketing/hero-split-image-06.ts";
...
const Primary = m("HeroSplitImage06Primary");
const EmailInput = m("HeroSplitImage06EmailInput", { email: S.String });
const Submit = m("HeroSplitImage06Submit");
const Secondary = m("HeroSplitImage06Secondary");
const Navigated = m("HeroSplitImage06Navigated", { href: S.String });
const MobileOpened = m("HeroSplitImage06MobileOpened");
const MobileClosed = m("HeroSplitImage06MobileClosed");
const AccountPressed = m("HeroSplitImage06AccountPressed");
const SearchChanged = m("HeroSplitImage06SearchChanged", { value: S.String });
type Message =
...
const definition = {
  ...
  update: (model: Model, message: Message): Model => {
    if (message._tag === "HeroSplitImage06MobileOpened") {
      return { ...model, isMobileOpen: true };
    }
    if (message._tag === "HeroSplitImage06MobileClosed") {
      return { ...model, isMobileOpen: false };
    }
    if (message._tag === "HeroSplitImage06SearchChanged") {
      return { ...model, searchValue: message.value };
    }
    return model;
  },
  view: (model: Model, h: Parameters<typeof heroSplitImage06<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [
        heroSplitImage06(
...
export default {
  ...componentMeta("hero-split-image-06"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Hero Split Image 06",
};
