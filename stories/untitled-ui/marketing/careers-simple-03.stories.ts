...
import { careersSimple03 } from "../../../../../packages/ui/src/marketing/careers-simple-03.ts";
...
const Selected = m("CareersSimple03JobSelected", { id: S.String });
...
const definition = {
  ...
  view: (model: Model, h: Parameters<typeof careersSimple03<Message>>[1]) =>
    h.div([h.Class("-m-8")], [careersSimple03({ ...model, onJob: (id) => Selected({ id }) }, h)]),
} as const;
const jobs = [
  {
    badgeColor: "blue",
...
export default {
  ...componentMeta("careers-simple-03"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Careers/Careers Simple 03",
};
...
export const Dark = {
  ...liveStory({
    ...
    view: (model, h) =>
      h.div(
        [h.Class("-m-8 min-h-[calc(100vh+4rem)] bg-bg-primary"), h.DataAttribute("theme", "dark")],
        [careersSimple03({ ...model, onJob: (id) => Selected({ id }) }, h)],
      ),
  }),
  args,
};
...
export const Interactions = {
  ...
  play: async ({ canvasElement }: { readonly canvasElement: HTMLElement }) => {
    ...
    const link = await within(canvasElement).findByRole("link", { name: /UX Researcher/u });
