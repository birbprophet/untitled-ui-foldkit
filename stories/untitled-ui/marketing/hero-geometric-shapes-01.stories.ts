...
import { heroGeometricShapes01 } from "../../../../../packages/ui/src/marketing/hero-geometric-shapes-01.ts";
...
const Primary = m("HeroGeometricShapes01Primary");
const Secondary = m("HeroGeometricShapes01Secondary");
const Navigated = m("HeroGeometricShapes01Navigated", { href: S.String });
const MobileOpened = m("HeroGeometricShapes01MobileOpened");
const MobileClosed = m("HeroGeometricShapes01MobileClosed");
const AccountPressed = m("HeroGeometricShapes01AccountPressed");
const SearchChanged = m("HeroGeometricShapes01SearchChanged", { value: S.String });
type Message =
  | typeof Primary.Type
...
const definition = {
  ...
  update: (model: Model, message: Message): Model => {
    if (message._tag === "HeroGeometricShapes01MobileOpened") {
      return { ...model, isMobileOpen: true };
    }
    if (message._tag === "HeroGeometricShapes01MobileClosed") {
      return { ...model, isMobileOpen: false };
    }
    if (message._tag === "HeroGeometricShapes01SearchChanged") {
      return { ...model, searchValue: message.value };
    }
    return model;
  },
  view: (model: Model, h: Parameters<typeof heroGeometricShapes01<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [
...
export default {
  ...componentMeta("hero-geometric-shapes-01"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Hero Geometric Shapes 01",
};
