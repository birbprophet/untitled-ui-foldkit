...
import { heroSplitImage03 } from "../../../../../packages/ui/src/marketing/hero-split-image-03.ts";
...
const Primary = m("HeroSplitImage03Primary");
const Secondary = m("HeroSplitImage03Secondary");
const Navigated = m("HeroSplitImage03Navigated", { href: S.String });
const MobileOpened = m("HeroSplitImage03MobileOpened");
const MobileClosed = m("HeroSplitImage03MobileClosed");
const AccountPressed = m("HeroSplitImage03AccountPressed");
const SearchChanged = m("HeroSplitImage03SearchChanged", { value: S.String });
type Message =
  | typeof Primary.Type
...
const definition = {
  ...
  update: (model: Model, message: Message): Model => {
    if (message._tag === "HeroSplitImage03MobileOpened") {
      return { ...model, isMobileOpen: true };
    }
    if (message._tag === "HeroSplitImage03MobileClosed") {
      return { ...model, isMobileOpen: false };
    }
    if (message._tag === "HeroSplitImage03SearchChanged") {
      return { ...model, searchValue: message.value };
    }
    return model;
  },
  view: (model: Model, h: Parameters<typeof heroSplitImage03<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [
        heroSplitImage03(
...
export default {
  ...componentMeta("hero-split-image-03"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Hero Split Image 03",
};
