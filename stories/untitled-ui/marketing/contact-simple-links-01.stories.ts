...
import { contactSimpleLinks01 } from "../../../../../packages/ui/src/marketing/contact-simple-links-01.ts";
...
const ContactSelected = m("ContactSimpleLinks01ContactSelected", { id: S.String });
...
const definition = {
  ...
  view: (model: Model, h: Parameters<typeof contactSimpleLinks01<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [contactSimpleLinks01({ ...model, onContact: (id) => ContactSelected({ id }) }, h)],
    ),
} as const;
...
export default {
  ...componentMeta("contact-simple-links-01"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Contact/Contact Simple Links 01",
};
...
export const Interactions = {
  ...
  play: async ({ canvasElement }: { readonly canvasElement: HTMLElement }) => {
    ...
    const office = await within(canvasElement).findByRole("link", { name: /100 Smith Street/u });
