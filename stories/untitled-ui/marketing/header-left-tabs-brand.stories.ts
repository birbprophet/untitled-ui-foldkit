...
const TabSelected = m("HeaderLeftTabsBrandTabSelected", { tabId: S.String });
...
const definition = {
  ...
  update: (model: Model, message: Message): Model =>
    message._tag === "HeaderLeftTabsBrandTabSelected"
      ? { ...model, selectedTabId: message.tabId }
      : model,
  view: (model: Model, h: Parameters<typeof headerLeftTabsBrand<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [headerLeftTabsBrand({ ...model, onTab: (tabId) => TabSelected({ tabId }) }, h)],
    ),
} as const;
...
export default {
  ...componentMeta("header-left-tabs-brand"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Header Left Tabs Brand",
};
