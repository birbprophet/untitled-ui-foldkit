...
import { headerLeftEmail } from "../../../../../packages/ui/src/marketing/header-left-email.ts";
...
const EmailInput = m("HeaderLeftEmailEmailInput", { email: S.String });
const Submit = m("HeaderLeftEmailSubmit");
...
const definition = {
  ...
  update: (model: Model, message: Message): Model =>
    message._tag === "HeaderLeftEmailEmailInput" ? { ...model, email: message.email } : model,
  view: (model: Model, h: Parameters<typeof headerLeftEmail<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [
        headerLeftEmail(
...
export default {
  ...componentMeta("header-left-email"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Header Left Email",
};
