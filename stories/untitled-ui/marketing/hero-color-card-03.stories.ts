...
import { heroColorCard03 } from "../../../../../packages/ui/src/marketing/hero-color-card-03.ts";
...
const Primary = m("HeroColorCard03Primary");
const EmailInput = m("HeroColorCard03EmailInput", { email: S.String });
const Submit = m("HeroColorCard03Submit");
const Secondary = m("HeroColorCard03Secondary");
const Navigated = m("HeroColorCard03Navigated", { href: S.String });
const MobileOpened = m("HeroColorCard03MobileOpened");
const MobileClosed = m("HeroColorCard03MobileClosed");
const AccountPressed = m("HeroColorCard03AccountPressed");
const SearchChanged = m("HeroColorCard03SearchChanged", { value: S.String });
type Message =
...
const definition = {
  ...
  update: (model: Model, message: Message): Model => {
    if (message._tag === "HeroColorCard03MobileOpened") {
      return { ...model, isMobileOpen: true };
    }
    if (message._tag === "HeroColorCard03MobileClosed") {
      return { ...model, isMobileOpen: false };
    }
    if (message._tag === "HeroColorCard03SearchChanged") {
      return { ...model, searchValue: message.value };
    }
    return model;
  },
  view: (model: Model, h: Parameters<typeof heroColorCard03<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [
        heroColorCard03(
...
export default {
  ...componentMeta("hero-color-card-03"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Hero Color Card 03",
};
