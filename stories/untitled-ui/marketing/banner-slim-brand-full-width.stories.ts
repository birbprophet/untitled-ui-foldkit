...
import { bannerSlimBrandFullWidth } from "../../../../../packages/ui/src/marketing/banner-slim-brand-full-width.ts";
...
const Dismissed = m("BannerSlimBrandFullWidthDismissed");
const LinkActivated = m("BannerSlimBrandFullWidthLinkActivated");
...
const definition = {
  ...
  update: (model: Model, message: Message): Model => {
    if (message._tag === "BannerSlimBrandFullWidthDismissed") {
      return { ...model, isVisible: false };
    }
    return { ...model, linkHref: "#dashboard-opened" };
  },
  view: (model: Model, h: Parameters<typeof bannerSlimBrandFullWidth<Message>>[1]) => {
    if (model.isVisible) {
      return bannerSlimBrandFullWidth({ ...model, onDismiss: dismissed, onLink: linkActivated }, h);
    }
...
export default {
  ...componentMeta("banner-slim-brand-full-width"),
  title: "Untitled UI/Marketing/Banners/Banner Slim Brand Full Width",
};
