...
import { headerLeftSearchBrand } from "../../../../../packages/ui/src/marketing/header-left-search-brand.ts";
...
const SearchInput = m("HeaderLeftSearchBrandSearchInput", { value: S.String });
...
const definition = {
  ...
  update: (model: Model, message: Message): Model =>
    message._tag === "HeaderLeftSearchBrandSearchInput"
      ? { ...model, searchValue: message.value }
      : model,
  view: (model: Model, h: Parameters<typeof headerLeftSearchBrand<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [headerLeftSearchBrand({ ...model, onSearch: (value) => SearchInput({ value }) }, h)],
    ),
} as const;
...
export default {
  ...componentMeta("header-left-search-brand"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Header Left Search Brand",
};
