...
import { bannerSlimBrand } from "../../../../../packages/ui/src/marketing/banner-slim-brand.ts";
...
const Dismissed = m("BannerSlimBrandDismissed");
const LinkActivated = m("BannerSlimBrandLinkActivated");
...
const definition = {
  ...
  update: (model: Model, message: Message): Model => {
    if (message._tag === "BannerSlimBrandDismissed") {
      return { ...model, isVisible: false };
    }
    return { ...model, linkHref: "#dashboard-opened" };
  },
  view: (model: Model, h: Parameters<typeof bannerSlimBrand<Message>>[1]) => {
    if (model.isVisible) {
      return bannerSlimBrand({ ...model, onDismiss: dismissed, onLink: linkActivated }, h);
    }
...
export default {
  ...componentMeta("banner-slim-brand"),
  title: "Untitled UI/Marketing/Banners/Banner Slim Brand",
};
