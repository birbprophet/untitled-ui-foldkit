...
import { headerCenteredEmail } from "../../../../../packages/ui/src/marketing/header-centered-email.ts";
...
const EmailInput = m("HeaderCenteredEmailEmailInput", { email: S.String });
const Submit = m("HeaderCenteredEmailSubmit");
...
const definition = {
  ...
  update: (model: Model, message: Message): Model =>
    message._tag === "HeaderCenteredEmailEmailInput" ? { ...model, email: message.email } : model,
  view: (model: Model, h: Parameters<typeof headerCenteredEmail<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [
        headerCenteredEmail(
...
export default {
  ...componentMeta("header-centered-email"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Header Centered Email",
};
