...
import { headerLeftTabs } from "../../../../../packages/ui/src/marketing/header-left-tabs.ts";
...
const TabSelected = m("HeaderLeftTabsTabSelected", { tabId: S.String });
...
const definition = {
  ...
  update: (model: Model, message: Message): Model =>
    message._tag === "HeaderLeftTabsTabSelected"
      ? { ...model, selectedTabId: message.tabId }
      : model,
  view: (model: Model, h: Parameters<typeof headerLeftTabs<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [headerLeftTabs({ ...model, onTab: (tabId) => TabSelected({ tabId }) }, h)],
    ),
} as const;
...
export default {
  ...componentMeta("header-left-tabs"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Header Left Tabs",
};
