...
const EmailInput = m("HeaderSpaceBetweenEmailBrandEmailInput", { email: S.String });
const Submit = m("HeaderSpaceBetweenEmailBrandSubmit");
...
const definition = {
  ...
  update: (model: Model, message: Message): Model =>
    message._tag === "HeaderSpaceBetweenEmailBrandEmailInput"
      ? { ...model, email: message.email }
      : model,
  view: (model: Model, h: Parameters<typeof headerSpaceBetweenEmailBrand<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [
        headerSpaceBetweenEmailBrand(
...
export default {
  ...componentMeta("header-space-between-email-brand"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Header Space Between Email Brand",
};
