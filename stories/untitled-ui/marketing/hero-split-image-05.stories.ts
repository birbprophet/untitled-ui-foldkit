...
import { heroSplitImage05 } from "../../../../../packages/ui/src/marketing/hero-split-image-05.ts";
...
const Primary = m("HeroSplitImage05Primary");
const Secondary = m("HeroSplitImage05Secondary");
const Navigated = m("HeroSplitImage05Navigated", { href: S.String });
const MobileOpened = m("HeroSplitImage05MobileOpened");
const MobileClosed = m("HeroSplitImage05MobileClosed");
const AccountPressed = m("HeroSplitImage05AccountPressed");
const SearchChanged = m("HeroSplitImage05SearchChanged", { value: S.String });
type Message =
  | typeof Primary.Type
...
const definition = {
  ...
  update: (model: Model, message: Message): Model => {
    if (message._tag === "HeroSplitImage05MobileOpened") {
      return { ...model, isMobileOpen: true };
    }
    if (message._tag === "HeroSplitImage05MobileClosed") {
      return { ...model, isMobileOpen: false };
    }
    if (message._tag === "HeroSplitImage05SearchChanged") {
      return { ...model, searchValue: message.value };
    }
    return model;
  },
  view: (model: Model, h: Parameters<typeof heroSplitImage05<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [
        heroSplitImage05(
...
export default {
  ...componentMeta("hero-split-image-05"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Hero Split Image 05",
};
