...
import { heroAbstractAngles01 } from "../../../../../packages/ui/src/marketing/hero-abstract-angles-01.ts";
...
const Primary = m("HeroAbstractAngles01Primary");
const Secondary = m("HeroAbstractAngles01Secondary");
const Navigated = m("HeroAbstractAngles01Navigated", { href: S.String });
const MobileOpened = m("HeroAbstractAngles01MobileOpened");
const MobileClosed = m("HeroAbstractAngles01MobileClosed");
const AccountPressed = m("HeroAbstractAngles01AccountPressed");
const SearchChanged = m("HeroAbstractAngles01SearchChanged", { value: S.String });
type Message =
  | typeof Primary.Type
...
const definition = {
  ...
  update: (model: Model, message: Message): Model => {
    if (message._tag === "HeroAbstractAngles01MobileOpened") {
      return { ...model, isMobileOpen: true };
    }
    if (message._tag === "HeroAbstractAngles01MobileClosed") {
      return { ...model, isMobileOpen: false };
    }
    if (message._tag === "HeroAbstractAngles01SearchChanged") {
      return { ...model, searchValue: message.value };
    }
    return model;
  },
  view: (model: Model, h: Parameters<typeof heroAbstractAngles01<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [
...
export default {
  ...componentMeta("hero-abstract-angles-01"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Hero Abstract Angles 01",
};
