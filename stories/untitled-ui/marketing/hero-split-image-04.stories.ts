...
import { heroSplitImage04 } from "../../../../../packages/ui/src/marketing/hero-split-image-04.ts";
...
const Primary = m("HeroSplitImage04Primary");
const EmailInput = m("HeroSplitImage04EmailInput", { email: S.String });
const Submit = m("HeroSplitImage04Submit");
const Secondary = m("HeroSplitImage04Secondary");
const Navigated = m("HeroSplitImage04Navigated", { href: S.String });
const MobileOpened = m("HeroSplitImage04MobileOpened");
const MobileClosed = m("HeroSplitImage04MobileClosed");
const AccountPressed = m("HeroSplitImage04AccountPressed");
const SearchChanged = m("HeroSplitImage04SearchChanged", { value: S.String });
type Message =
...
const definition = {
  ...
  update: (model: Model, message: Message): Model => {
    if (message._tag === "HeroSplitImage04MobileOpened") {
      return { ...model, isMobileOpen: true };
    }
    if (message._tag === "HeroSplitImage04MobileClosed") {
      return { ...model, isMobileOpen: false };
    }
    if (message._tag === "HeroSplitImage04SearchChanged") {
      return { ...model, searchValue: message.value };
    }
    return model;
  },
  view: (model: Model, h: Parameters<typeof heroSplitImage04<Message>>[1]) =>
    h.div(
      [h.Class("-m-8")],
      [
        heroSplitImage04(
...
export default {
  ...componentMeta("hero-split-image-04"),
  parameters: { layout: "fullscreen" },
  title: "Untitled UI/Marketing/Header Section/Hero Split Image 04",
};
