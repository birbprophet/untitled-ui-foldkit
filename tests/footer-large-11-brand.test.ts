...
import type { FooterLarge11BrandProps } from "../src/marketing/footer-large-11-brand.ts";

describe("footer large 11 brand", () => {
  it("keeps footer navigation controlled", () => {
    ...
    } satisfies FooterLarge11BrandProps<string>;

    expect(props.onLink("overview")).toBe("link:overview");
    expect(props.onSocial("x")).toBe("social:x");
    expect(footerLarge11Brand).toBeTypeOf("function");
  });
});
