...
import type { FooterLarge08BrandProps } from "../src/marketing/footer-large-08-brand.ts";

describe("footer large 08 brand", () => {
  it("keeps footer navigation controlled", () => {
    ...
    } satisfies FooterLarge08BrandProps<string>;

    expect(props.onLink("overview")).toBe("link:overview");
    expect(props.onSocial("x")).toBe("social:x");
    expect(footerLarge08Brand).toBeTypeOf("function");
  });
});
