...
import type { FooterLarge05BrandProps } from "../src/marketing/footer-large-05-brand.ts";

describe("footer large 05 brand", () => {
  it("keeps footer navigation controlled", () => {
    ...
    } satisfies FooterLarge05BrandProps<string>;

    expect(props.onLink("overview")).toBe("link:overview");
    expect(props.onSocial("x")).toBe("social:x");
    expect(footerLarge05Brand).toBeTypeOf("function");
  });
});
