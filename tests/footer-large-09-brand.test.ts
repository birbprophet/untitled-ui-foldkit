...
import type { FooterLarge09BrandProps } from "../src/marketing/footer-large-09-brand.ts";

describe("footer large 09 brand", () => {
  it("keeps footer navigation controlled", () => {
    ...
    } satisfies FooterLarge09BrandProps<string>;

    expect(props.onLink("overview")).toBe("link:overview");
    expect(props.onSocial("x")).toBe("social:x");
    expect(footerLarge09Brand).toBeTypeOf("function");
  });
});
