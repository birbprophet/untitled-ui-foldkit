...
import type { FooterLarge10BrandProps } from "../src/marketing/footer-large-10-brand.ts";

describe("footer large 10 brand", () => {
  it("keeps footer navigation controlled", () => {
    ...
    } satisfies FooterLarge10BrandProps<string>;

    expect(props.onLink("overview")).toBe("link:overview");
    expect(props.onSocial("x")).toBe("social:x");
    expect(footerLarge10Brand).toBeTypeOf("function");
  });
});
