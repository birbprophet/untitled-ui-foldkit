...
import type { FooterLarge12BrandProps } from "../src/marketing/footer-large-12-brand.ts";

describe("footer large 12 brand", () => {
  it("keeps footer navigation controlled", () => {
    ...
    } satisfies FooterLarge12BrandProps<string>;

    expect(props.onLink("overview")).toBe("link:overview");
    expect(props.onSocial("x")).toBe("social:x");
    expect(footerLarge12Brand).toBeTypeOf("function");
  });
});
