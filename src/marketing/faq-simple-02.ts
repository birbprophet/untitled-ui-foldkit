...
export type FaqSimple02Icon = "credit-card" | "file" | "heart" | "mail" | "slash" | "switch";

export interface FaqSimple02Item {
  readonly answer: string;
  readonly icon: FaqSimple02Icon;
  readonly id: string;
  readonly question: string;
}

export interface FaqSimple02Props<Message> {
  readonly ctaDescription: string;
  readonly ctaHeading: string;
  readonly ctaLabel: string;
  readonly description: string;
  readonly heading: string;
  readonly items: readonly FaqSimple02Item[];
  readonly onCta: NoInfer<Message>;
  readonly onSupport: NoInfer<Message>;
  readonly supportHref: string;
  readonly supportLabel: string;
}

export const faqSimple02DefaultItems = [
  {
...
] as const satisfies readonly FaqSimple02Item[];

const iconPaths: Record<FaqSimple02Icon, readonly string[]> = {
  "credit-card": [
...
export const faqSimple02 = <Message>(
  props: FaqSimple02Props<Message>,
  h: HtmlBuilder<Message>,
): Html => {
      ...
      h.div(
        ...
        [
          h.div(
            ...
            [
              ...
              h.a(
                [
                  h.Class(
                    "mt-4 rounded-xs underline underline-offset-4 outline-focus-ring focus-visible:outline-2 focus-visible:outline-offset-2",
                  ),
                  h.Href(props.supportHref),
                  h.OnClick(props.onSupport),
                ],
                [props.supportLabel],
              ),
            ],
          ...
          h.div(
            [
              h.Class(
                "mt-12 flex flex-col items-start justify-between gap-6 rounded-2xl bg-bg-secondary px-5 py-8 md:mt-16 md:flex-row md:gap-8 md:p-8",
              ),
            ],
            [
              h.div(
