...
import { marketingMenuColumnList, marketingMenuReadyFooter, type MarketingMenuFooterAction } from "./marketing-menu-shared.ts";
...
export const menu4ColWithFooter = <Message>(props: Menu4ColWithFooterProps<Message>, h: HtmlBuilder<Message>): Html =>
  h.section([h.Class("relative overflow-hidden bg-bg-primary md:shadow-lg"), h.Dir("ltr")], [
    h.div([h.Class("mx-auto max-w-container md:px-8")], [
    ...
    marketingMenuReadyFooter(
      { actions: props.actions, ctaLabel: props.ctaLabel, onAction: props.onAction, onCta: props.onCta, prompt: props.prompt },
      h,
    ),
  ]);
