/* oxlint-disable effect/noReturnInArrow, foldkit/keyed-required-for-mapped-rows -- This section directly transcribes the authenticated Untitled UI contact cards. */
import type { Html, HtmlBuilder } from "foldkit/html";

export type ContactIconCards01Icon = "chat" | "location" | "phone" | "smile";

export interface ContactIconCards01Card {
  readonly cta: string;
  readonly href: string;
  readonly icon: ContactIconCards01Icon;
  readonly id: string;
  readonly subtitle: string;
  readonly title: string;
}

export interface ContactIconCards01Props<Message> {
  readonly cards: readonly ContactIconCards01Card[];
...
export const contactIconCards01Cards = [
  {
...
] as const satisfies readonly ContactIconCards01Card[];

const iconPath: Record<ContactIconCards01Icon, readonly string[]> = {
  chat: [
...
const featuredIcon = <Message>(kind: ContactIconCards01Icon, h: HtmlBuilder<Message>): Html =>
  h.div(
    [
      h.Class(
...
const contactCard = <Message>(
  card: ContactIconCards01Card,
  onActivate: (id: string) => NoInfer<Message>,
  ...
  h.li(
    ...
    [
      ...
      h.h3([h.Class("mt-12 text-lg font-semibold text-text-primary md:mt-16")], [card.title]),
...
export const contactIconCards01 = <Message>(
  props: ContactIconCards01Props<Message>,
  h: HtmlBuilder<Message>,
): Html =>
  h.section(
      ...
      h.div(
        ...
        [
          ...
          h.div(
            ...
            [
              h.ul(
                [h.Class("grid w-full grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4")],
                props.cards.map((card) => contactCard(card, props.onActivate, h)),
              ),
            ],
          ),
        ],
