...
export interface FeaturesSimpleIcons02BrandItem {
  readonly icon:
    | "chat"
    | "zap"
    | "chart"
...
export interface FeaturesSimpleIcons02BrandProps<Message> {
  readonly description: string;
  readonly eyebrow: string;
  readonly heading: string;
  readonly items: readonly FeaturesSimpleIcons02BrandItem[];
...
const iconPaths: Record<FeaturesSimpleIcons02BrandItem["icon"], readonly string[]> = {
  chart: [
...
const featuredIcon = <Message>(
  kind: FeaturesSimpleIcons02BrandItem["icon"],
  size: "md" | "lg",
  theme: "light" | "dark",
...
const featureItem = <Message>(
  feature: FeaturesSimpleIcons02BrandItem,
  ...
  h.li(
    ...
    [
      featuredIcon(feature.icon, "lg", "dark", h),
      featuredIcon(feature.icon, "md", "dark", h),
      h.h3([h.Class("mt-4 text-lg font-semibold text-primary_on-brand md:mt-5")], [feature.title]),
      h.p([h.Class("mt-1 text-md text-tertiary_on-brand")], [feature.subtitle]),
      ...(onItem === undefined
        ? []
        : [
            h.button(
...
export const featuresSimpleIcons02Brand = <Message>(
  props: FeaturesSimpleIcons02BrandProps<Message>,
  h: HtmlBuilder<Message>,
): Html =>
  h.section(
