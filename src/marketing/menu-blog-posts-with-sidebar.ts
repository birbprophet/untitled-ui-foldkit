...
import { marketingMenuBlogPostHorizontal } from "./marketing-menu-shared.ts";
import type { MarketingMenuBlogPost, MarketingMenuCategoryItem } from "./marketing-menu-shared.ts";
...
export const menuBlogPostsWithSidebar = <Message>(
  ...
  return h.section(
    ...
    [
      h.div(
        [h.Class("mx-auto flex max-w-container flex-col md:flex-row md:gap-16 md:px-8")],
        [
          h.div(
            [h.Class("relative z-10 flex min-w-50 flex-col gap-3 py-4 md:pt-6 md:pb-8")],
            [
              h.h3(
                [h.Class("px-4 text-sm font-semibold text-text-brand-tertiary md:px-0")],
          ...
          h.ul(
            ...
            visiblePosts.map((post) =>
              h.keyed("li")(post.id, [], [marketingMenuBlogPostHorizontal(post, h)]),
            ),
          ),
        ],
      ),
