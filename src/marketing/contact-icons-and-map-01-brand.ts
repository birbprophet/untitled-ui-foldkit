...
export type ContactIconsAndMap01BrandIcon = "email" | "office" | "phone";

export interface ContactIconsAndMap01BrandItem {
  readonly cta: string;
  readonly href: string;
  readonly icon: ContactIconsAndMap01BrandIcon;
  readonly id: string;
  readonly subtitle: string;
  readonly title: string;
}

export interface ContactIconsAndMap01BrandProps<Message> {
  readonly description: string;
  readonly eyebrow: string;
  readonly heading: string;
  readonly items: readonly ContactIconsAndMap01BrandItem[];
  readonly mapSrc: string;
  readonly mapTitle: string;
  readonly onContact: (id: string) => NoInfer<Message>;
}

export const contactIconsAndMap01BrandItems = [
  {
    cta: "hi@siglata.com",
...
] as const satisfies readonly ContactIconsAndMap01BrandItem[];

const iconPaths: Record<ContactIconsAndMap01BrandIcon, readonly string[]> = {
  email: [
...
const featuredIcon = <Message>(
  icon: ContactIconsAndMap01BrandIcon,
...
const contactItem = <Message>(
  contact: ContactIconsAndMap01BrandItem,
  ...
  h.keyed("li")(
    ...
    [
      ...
      h.div(
        [h.Class("lg:pt-2.5")],
        [
          h.h3([h.Class("text-lg font-semibold text-primary_on-brand")], [contact.title]),
          h.p([h.Class("mt-1 text-md text-tertiary_on-brand")], [contact.subtitle]),
          h.a(
            [
              h.Class(
                "group relative mt-4 inline-flex h-max cursor-pointer items-center justify-normal gap-1.5 rounded p-0! text-md font-semibold whitespace-pre text-primary_on-brand outline-focus-ring transition duration-100 ease-linear before:absolute before:rounded-[7px] hover:text-text-tertiary-hover focus-visible:outline-2 focus-visible:outline-offset-2 *:data-icon:pointer-events-none *:data-icon:size-5 *:data-icon:shrink-0 *:data-icon:text-fg-quaternary *:data-icon:transition-inherit-all *:data-text:underline *:data-text:decoration-transparent *:data-text:underline-offset-4 hover:*:data-icon:text-fg-quaternary-hover hover:*:data-text:decoration-fg-quaternary lg:mt-5",
              ),
              h.Href(contact.href),
...
export const contactIconsAndMap01Brand = <Message>(
  props: ContactIconsAndMap01BrandProps<Message>,
  h: HtmlBuilder<Message>,
): Html =>
  h.section(
