...
export type FaqSimple04Icon = "credit-card" | "file" | "heart" | "mail" | "slash" | "switch";

export interface FaqSimple04Item {
  readonly answer: string;
  readonly icon: FaqSimple04Icon;
  readonly id: string;
  readonly question: string;
}

export interface FaqSimple04Avatar {
  readonly alt: string;
...
export interface FaqSimple04Props<Message> {
  readonly ctaDescription: string;
  readonly ctaHeading: string;
  readonly ctaLabel: string;
  readonly description: string;
  readonly heading: string;
  readonly items: readonly FaqSimple04Item[];
  readonly onCta: NoInfer<Message>;
  readonly avatars: readonly FaqSimple04Avatar[];
}

export const faqSimple04DefaultItems = [
  {
...
] as const satisfies readonly FaqSimple04Item[];

const iconPaths: Record<FaqSimple04Icon, readonly string[]> = {
  "credit-card": [
  ...
  h.div(
    [
      h.Class(
        ...
        } items-center justify-center rounded-lg bg-bg-secondary text-fg-quaternary ring-1 ring-border-secondary ring-inset`,
      ),
...
const avatarNode = <Message>(entry: FaqSimple04Avatar, h: HtmlBuilder<Message>): Html =>
  avatar({ alt: entry.alt, size: entry.size, src: entry.src }, h);

export const faqSimple04 = <Message>(
  props: FaqSimple04Props<Message>,
  h: HtmlBuilder<Message>,
): Html => {
      ...
      h.div(
        ...
        [
          ...
          h.div(
            [
              h.Class(
                "mt-12 flex flex-col items-center gap-6 rounded-2xl bg-bg-secondary px-6 py-8 text-center md:mt-16 md:gap-8 md:px-8 md:py-8 md:pb-10",
              ),
            ],
            [
              h.div(
                    ...
                    [
                      h.Class(
                        member.emphasis === true
                          ? "z-10 ring-[1.5px] ring-fg-white"
                          : "ring-[1.5px] ring-fg-white",
                      ),
                    ],
                    [avatarNode(member, h)],
                  ),
