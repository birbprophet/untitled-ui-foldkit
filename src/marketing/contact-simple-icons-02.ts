...
export type ContactSimpleIcons02Icon = "mail" | "phone" | "pin";

export interface ContactSimpleIcons02Method {
  readonly cta: string;
  readonly href: string;
  readonly icon: ContactSimpleIcons02Icon;
  readonly id: string;
  readonly subtitle: string;
  readonly title: string;
}

export interface ContactSimpleIcons02Props<Message> {
  readonly description: string;
  readonly eyebrow: string;
  readonly heading: string;
  readonly methods: readonly ContactSimpleIcons02Method[];
  readonly onContact: (id: string) => NoInfer<Message>;
}

const iconPaths: Readonly<Record<ContactSimpleIcons02Icon, readonly string[]>> = {
  mail: [
...
export const contactSimpleIcons02 = <Message>(
  props: ContactSimpleIcons02Props<Message>,
  h: HtmlBuilder<Message>,
): Html =>
  h.section(
                ...
                props.methods.map((method) =>
                  h.keyed("li")(
                    ...
                    [
                      ...
                      h.h3(
                        [h.Class("mt-4 text-lg font-semibold text-primary md:mt-5")],
                        [method.title],
                      ),
                      ...
                      h.a(
                        [
                          h.Class(
                            "mt-4 inline-flex h-max cursor-pointer items-center justify-normal whitespace-pre rounded p-0! text-md font-semibold text-text-brand-secondary outline-focus-ring transition duration-100 ease-linear hover:text-text-brand-secondary-hover focus-visible:outline-2 focus-visible:outline-offset-2 md:mt-5",
                          ),
                          h.Href(method.href),
                          h.OnClick(props.onContact(method.id)),
                        ],
                        [method.cta],
                      ),
                    ],
