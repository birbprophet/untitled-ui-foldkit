...
const desktopNavItem = <Message>(
  ...
  if (navItem.hasMenu === true) {
    const expanded = props.openMenuId === navItem.id;
    return h.button(
      [
        ...
        h.OnClick(props.onMenuToggle(navItem.id)),
        h.Type("button"),
      ],
      [h.span([h.Class("px-0.5")], [navItem.label]), chevronDown(expanded, h)],
    );
  }
  ...
  return h.a(
    [
      ...
      h.Href(navItem.href ?? "#"),
      h.OnClick(props.onItem(navItem.id)),
    ],
    [h.span([h.Class("px-0.5")], [navItem.label])],
  );
};
...
const mobileNavItem = <Message>(
    ...
    return h.li(
      ...
      [
        h.a(
          [
            h.Class(
              "flex items-center justify-between px-4 py-3 text-sm font-semibold text-text-primary hover:bg-bg-primary_hover",
            ),
            h.Href(navItem.href),
            h.OnClick(props.onItem(navItem.id)),
          ],
  ...
  const expanded = props.mobileExpandedItemId === navItem.id;
  const expandedChildren = expanded
    ? [
        h.button(
          [
            h.AriaExpanded(expanded),
            ...
            h.OnClick(props.onMobileExpand(navItem.id)),
            h.Type("button"),
          ],
          [navItem.label, chevronDown(expanded, h)],
        ),
        h.div([h.Class("px-2 pb-2")]),
      ]
    : [
        h.button(
          [
  ...
  return h.li([h.Class("flex flex-col gap-0.5")], expandedChildren);
};

export const header = <Message>(props: HeaderProps<Message>, h: HtmlBuilder<Message>): Html => {
  const mobileMenu = props.isMobileOpen
    ? h.nav(
        ...
        [
          ...
          h.div(
            [h.Class("flex flex-col gap-8 border-t border-border-secondary px-4 py-6")],
            [
              h.ul(
              ...
              h.div(
                ...
                [
                  button(
                    { color: "primary", label: "Sign up", onPress: props.onSignup, size: "md" },
                    h,
                  ),
                  button(
                    { color: "secondary", label: "Log in", onPress: props.onLogin, size: "md" },
                    h,
                  ),
                ],
              ),
  ...
  return h.header(
    ...
    [
      h.div(
        [h.Class("flex size-full max-w-container flex-1 items-center pr-3 pl-4 md:px-8")],
        [
          h.div(
            [
              h.Class(
                    ...
                    [
                      h.ul(
                        ...
                        props.items.map((navItem) =>
                          h.keyed("li")(navItem.id, [], [desktopNavItem(navItem, props, h)]),
                        ),
                      ),
                    ],
                  ),
