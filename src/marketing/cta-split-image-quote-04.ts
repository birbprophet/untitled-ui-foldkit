...
export interface CtaSplitImageQuote04Props<Message> {
...
export const ctaSplitImageQuote04 = <Message>(props: CtaSplitImageQuote04Props<Message>, h: HtmlBuilder<Message>): Html =>
  h.section([h.Class("bg-bg-primary py-16 md:py-24"), h.Dir("ltr")], [
    h.div([h.Class("mx-auto grid max-w-container grid-cols-1 items-center gap-16 px-4 md:px-8 lg:grid-cols-2")], [h.img([h.Alt(props.imageAlt), h.Class("h-70 w-full object-cover md:h-140"), h.Src(props.imageSrc)]), h.div([h.Class("flex max-w-3xl flex-col items-start")], [
