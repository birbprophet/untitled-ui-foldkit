...
export type ContactSimpleIcons02BrandIcon = "email" | "office" | "phone";

export interface ContactSimpleIcons02BrandItem {
  readonly cta: string;
  readonly href: string;
  readonly icon: ContactSimpleIcons02BrandIcon;
  readonly id: string;
  readonly subtitle: string;
  readonly title: string;
}

export interface ContactSimpleIcons02BrandProps<Message> {
  readonly description: string;
  readonly eyebrow: string;
  readonly heading: string;
  readonly items: readonly ContactSimpleIcons02BrandItem[];
  readonly onContact: (id: string) => NoInfer<Message>;
}

export const contactSimpleIcons02BrandItems = [
  {
    cta: "hi@siglata.com",
...
] as const satisfies readonly ContactSimpleIcons02BrandItem[];

const iconPaths: Record<ContactSimpleIcons02BrandIcon, readonly string[]> = {
  email: [
...
const featuredIcon = <Message>(
  icon: ContactSimpleIcons02BrandIcon,
  size: "md" | "lg",
  ...
  h.div(
    [
      h.Class(
        size === "md"
          ? "relative flex size-10 shrink-0 items-center justify-center rounded-lg bg-bg-brand-solid text-fg-white shadow-xs-skeuomorphic before:absolute before:inset-px before:rounded-[7px] before:border before:border-utility-brand-200/12 before:mask-b-from-0% md:hidden"
          : "relative hidden size-12 shrink-0 items-center justify-center rounded-[10px] bg-bg-brand-solid text-fg-white shadow-xs-skeuomorphic before:absolute before:inset-px before:rounded-[9px] before:border before:border-utility-brand-200/12 before:mask-b-from-0% md:flex",
      ),
    ...
    [
      h.svg(
        [
          h.AriaHidden(true),
          h.Class(size === "md" ? "z-1 size-5" : "z-1 size-6"),
...
const contactItem = <Message>(
  contact: ContactSimpleIcons02BrandItem,
  ...
  h.keyed("li")(
    ...
    [
      featuredIcon(contact.icon, "md", h),
      featuredIcon(contact.icon, "lg", h),
      ...
      h.a(
        [
          h.Class(
            "group relative mt-4 inline-flex h-max cursor-pointer items-center justify-normal gap-1.5 whitespace-pre rounded p-0! text-md font-semibold text-primary_on-brand outline-focus-ring transition duration-100 ease-linear before:absolute before:rounded-[7px] hover:text-text-tertiary-hover focus-visible:outline-2 focus-visible:outline-offset-2 *:data-text:underline *:data-text:decoration-transparent *:data-text:underline-offset-4 hover:*:data-text:decoration-fg-quaternary md:mt-5",
          ),
          h.Href(contact.href),
...
export const contactSimpleIcons02Brand = <Message>(
  props: ContactSimpleIcons02BrandProps<Message>,
  h: HtmlBuilder<Message>,
): Html =>
  h.section(
          ...
          h.div(
            ...
            [
              h.ul(
                ...
                props.items.map((contact) => contactItem(contact, props.onContact, h)),
              ),
            ],
          ),
        ],
