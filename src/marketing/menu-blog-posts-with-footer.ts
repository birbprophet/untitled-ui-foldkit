...
import type { MarketingMenuBlogPost } from "./marketing-menu-shared.ts";
...
export const menuBlogPostsWithFooter = <Message>(
  ...
  return h.section(
    ...
    [
      h.div(
        [h.Class(`mx-auto max-w-container ${props.isFloating === true ? "md:px-5" : "md:px-8"}`)],
        [
          h.ul(
      ...
      h.div(
        [
          h.Class(
            `bg-bg-secondary ${props.isFloating === true ? "md:mx-2 md:mb-2 md:rounded-lg" : ""}`,
          ),
        ],
        [
          h.div(
