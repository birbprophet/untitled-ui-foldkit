/* oxlint-disable effect/noReturnInArrow -- This renderer is a direct FoldKit transcription of the authenticated Untitled UI dual-action brand banner. */
import type { Html, HtmlBuilder } from "foldkit/html";

import { button } from "../base/button.ts";

export interface BannerDualActionBrandProps<Message> {
...
const verifiedIcon = <Message>(h: HtmlBuilder<Message>): Html =>
  h.svg(
    [
      h.AriaHidden(true),
      h.Class("size-5 text-icon-fg-brand_on-brand"),
      h.Fill("none"),
      h.ViewBox("0 0 24 24"),
    ],
    [
...
export const bannerDualActionBrand = <Message>(
  props: BannerDualActionBrandProps<Message>,
  h: HtmlBuilder<Message>,
): Html =>
  h.div(
    [
      h.Class(
        "relative mx-2 mb-4 flex flex-col gap-4 rounded-xl border-t border-brand_alt bg-brand-section_subtle p-4 shadow-lg md:m-0 md:flex-row md:items-center md:gap-3 md:border-t-0 md:border-b md:py-3 md:pr-3 md:pl-5",
      ),
      h.Dir("ltr"),
    ],
    [
