/* oxlint-disable effect/noReturnInArrow, effect/noSpread, effect/noTernary -- The authenticated upstream variant and logo tables stay explicit. */
import type { Html, HtmlBuilder } from "foldkit/html";

export type SocialButtonNetwork = "google" | "facebook" | "apple" | "twitter" | "figma" | "dribble";
export type SocialButtonSize = "md" | "lg";
export type SocialButtonTheme = "brand" | "color" | "gray";

export interface SocialButtonProps<Message> {
  ...
  readonly social: SocialButtonNetwork;
  readonly theme?: SocialButtonTheme;
}

const google = <Message>(colorful: boolean, h: HtmlBuilder<Message>): Html =>
  h.svg(
    [h.Fill("none"), h.ViewBox("0 0 24 24")],
    [
      h.path([
        h.D(
          "M23.766 12.2764C23.766 11.4607 23.6999 10.6406 23.5588 9.83807H12.24V14.4591H18.7217C18.4528 15.9494 17.5885 17.2678 16.323 18.1056V21.1039H20.19C22.4608 19.0139 23.766 15.9274 23.766 12.2764Z",
        ),
        h.Fill(colorful ? "#4285F4" : "currentColor"),
      ]),
      h.path([
        h.D(
          "M12.24 24.0008C15.4764 24.0008 18.2058 22.9382 20.1944 21.1039L16.3274 18.1055C15.2516 18.8375 13.8626 19.252 12.2444 19.252C9.11376 19.252 6.45934 17.1399 5.50693 14.3003H1.51648V17.3912C3.55359 21.4434 7.70278 24.0008 12.24 24.0008Z",
        ),
        h.Fill(colorful ? "#34A853" : "currentColor"),
      ]),
      h.path([
        h.D(
          "M5.50253 14.3003C4.99987 12.8099 4.99987 11.1961 5.50253 9.70575V6.61481H1.51649C-0.18551 10.0056-0.18551 14.0004 1.51649 17.3912L5.50253 14.3003Z",
        ),
        h.Fill(colorful ? "#FBBC04" : "currentColor"),
      ]),
      h.path([
        h.D(
          "M12.24 4.74966C13.9508 4.7232 15.6043 5.36697 16.8433 6.54867L20.2694 3.12262C18.1 1.0855 15.2207-0.034466 12.24 0.000808666C7.70277 0.000808666 3.55359 2.55822 1.51648 6.61481L5.50252 9.70575C6.45052 6.86173 9.10935 4.74966 12.24 4.74966Z",
        ),
        h.Fill(colorful ? "#EA4335" : "currentColor"),
      ]),
    ],
  );

const figma = <Message>(colorful: boolean, outlined: boolean, h: HtmlBuilder<Message>): Html => {
  if (outlined) {
    return h.svg(
      [h.Fill("none"), h.ViewBox("0 0 24 24")],
      [
  ...
  const colors = colorful
    ? ["#24CB71", "#874FFF", "#FF3737", "#FF7237", "#00B6FF"]
    : ["currentColor", "currentColor", "currentColor", "currentColor", "currentColor"];
  const paths = [
    "M8.00006 24.0001C10.2081 24.0001 12.0001 22.208 12.0001 20V16H8.00006C5.79205 16 4 17.792 4 20C4 22.208 5.79205 24.0001 8.00006 24.0001Z",
    "M4 12C4 9.79203 5.79205 8 8.00006 8H12.0001V16H8.00006C5.79205 16.0001 4 14.208 4 12Z",
    "M4 4.00003C4 1.79203 5.79205 0 8.00006 0H12.0001V7.99997H8.00006C5.79205 7.99997 4 6.20803 4 4.00003Z",
    "M12 0H16.0001C18.2081 0 20.0001 1.79203 20.0001 4.00003C20.0001 6.20803 18.2081 7.99997 16.0001 7.99997H12V0Z",
    "M20.0001 12C20.0001 14.208 18.2081 16.0001 16.0001 16.0001C13.792 16.0001 12 14.208 12 12C12 9.79203 13.792 8 16.0001 8C18.2081 8 20.0001 9.79203 20.0001 12Z",
  ];
  return h.svg(
    [h.Fill("none"), h.ViewBox("0 0 24 24")],
    paths.map((path, index) => h.path([h.D(path), h.Fill(colors[index] ?? "currentColor")])),
  );
};

const singlePathLogo = <Message>(
  network: Exclude<SocialButtonNetwork, "google" | "figma">,
  colorful: boolean,
  ...
  if (network === "dribble") {
    return h.svg(
      ...
      [
        h.path([
          h.D(
            "M12 23.625C18.4203 23.625 23.625 18.4203 23.625 12C23.625 5.57969 18.4203 0.375 12 0.375C5.57969 0.375 0.375 5.57969 0.375 12C0.375 18.4203 5.57969 23.625 12 23.625Z",
          ),
          h.Fill(colorful ? "#EA4C89" : "none"),
        ]),
        h.path([
          h.ClipRule("evenodd"),
          h.D(
          ...
          h.Fill(colorful ? "#C32361" : "currentColor"),
          h.FillRule("evenodd"),
        ]),
      ],
    );
  ...
  const paths = {
    ...
    twitter:
      "M15.9455 23L10.396 15.0901L3.44886 23H0.509766L9.09209 13.2311L0.509766 1H8.05571L13.286 8.45502L19.8393 1H22.7784L14.5943 10.3165L23.4914 23H15.9455ZM19.2185 20.77H17.2398L4.71811 3.23H6.6971L11.7121 10.2532L12.5793 11.4719L19.2185 20.77Z",
  } as const;
  const path = paths[network];
  return h.svg(
    [h.Fill("none"), h.ViewBox("0 0 24 24")],
    [h.path([h.ClipRule("evenodd"), h.D(path), h.Fill("currentColor"), h.FillRule("evenodd")])],
  );
};

const logo = <Message>(
  social: SocialButtonNetwork,
  ...
  let icon: Html;
  if (social === "google") {
    icon = google(colorful, h);
  } else if (social === "figma") {
    icon = figma(colorful, theme === "gray", h);
  } else {
    icon = singlePathLogo(social, colorful, h);
  }
  let color = "";
  if (theme === "gray") {
    color = "text-fg-quaternary group-hover:text-fg-quaternary-hover";
  } else if (theme === "brand" && ["facebook", "apple", "twitter"].includes(social)) {
    color = "text-white";
  } else if (theme === "color" && social === "facebook") {
    color = "text-[#1877F2]";
  } else if (theme === "color" && (social === "apple" || social === "twitter")) {
    color = "text-alpha-black";
  }
  return h.span(
    [
      h.Class(
        `pointer-events-none inline-flex shrink-0 transition-inherit-all [&>svg]:size-full ${size === "md" ? "size-4" : "size-5"} ${color}`,
      ),
    ],
    [icon],
  );
...
export const socialButton = <Message>(
  props: SocialButtonProps<Message>,
  h: HtmlBuilder<Message>,
): Html => {
  ...
  const brandColor = {
    ...
    dribble: "bg-[#EA4C89] text-white hover:bg-[#E62872]",
    facebook: "bg-[#1877F2] text-white hover:bg-[#0C63D4]",
    figma: "bg-black text-white",
    google:
      "bg-bg-primary text-text-secondary ring-border-primary hover:bg-bg-primary-hover hover:text-text-secondary-hover",
  ...
  const color =
    theme === "brand"
      ? brandColor
      : "bg-bg-primary text-text-secondary ring-border-primary hover:bg-bg-primary-hover hover:text-text-secondary-hover";
  const classes = `group relative inline-flex h-max cursor-pointer items-center justify-center whitespace-nowrap font-semibold shadow-xs-skeuomorphic ring-1 ring-transparent outline-focus-ring transition duration-100 ease-linear before:absolute focus-visible:outline-2 focus-visible:outline-offset-2 ring-inset disabled:cursor-not-allowed disabled:text-fg-disabled ${theme === "brand" && props.social !== "google" ? "before:inset-px before:border before:border-white/12 before:mask-b-from-0%" : ""} ${size === "md" ? "gap-2 rounded-lg px-3.5 py-2.5 text-sm before:rounded-[7px] data-icon-only:p-3" : "gap-2.5 rounded-lg px-4 py-2.5 text-md before:rounded-[7px] data-icon-only:p-3"} ${color} ${disabled && props.href !== undefined ? "pointer-events-none cursor-not-allowed text-fg-disabled" : ""}`;
  const children = [
    logo(props.social, theme, size, h),
    ...(props.label === undefined ? [] : [h.span([], [props.label])]),
  ];
  const attributes = [
    h.Class(classes),
    h.AriaLabel(props.accessibleLabel),
    ...(props.label === undefined ? [h.DataAttribute("icon-only", "true")] : []),
  ];
  if (props.href !== undefined) {
    return h.a(
      [...attributes, h.AriaDisabled(disabled), ...(disabled ? [] : [h.Href(props.href)])],
      children,
    );
  }
  return h.button(
