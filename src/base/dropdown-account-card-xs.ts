...
const orderedIds = ["settings", "dark-mode", "olivia", "sienna", "add-account", "sign-out"];
...
const moveFocus = <Message>(
  props: DropdownAccountCardXSProps<Message>,
  id: string,
  key: string,
) => {
  ...
  return next === undefined
    ? Option.none()
    : Option.some({
        focusSelector: `[data-account-card-xs-item="${next}"]`,
  ...
  return h.button(
    [
      ...
      ...(options?.submenu === true
        ? [h.AriaHasPopup("menu"), h.AriaExpanded(props.isSignOutOpen)]
        : []),
      h.Tabindex(props.focusedId === id ? 0 : -1),
      h.DataAttribute("account-card-xs-item", id),
...
export const dropdownAccountCardXS = <Message>(
  props: DropdownAccountCardXSProps<Message>,
  h: HtmlBuilder<Message>,
): Html =>
  h.div(
    ...
    [
      h.button(
        [
          h.Class(
            `relative flex w-38 cursor-pointer items-center gap-1.5 rounded-lg bg-bg-primary-alt p-2 text-left ring-1 ring-border-secondary ring-inset outline-offset-2 outline-focus-ring ${props.isOpen ? "outline-2" : ""}`,
          ),
          h.Type("button"),
          h.AriaHasPopup("menu"),
      ...
      h.div(
        [
          h.Class(
            `${props.isOpen ? "" : "hidden"} absolute top-full right-0 z-20 mt-2 w-50 origin-top-right overflow-visible rounded-lg bg-bg-primary py-1 shadow-lg ring-1 ring-border-secondary-alt`,
          ),
          h.Role("menu"),
        ],
        [
