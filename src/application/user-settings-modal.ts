...
const cloudsUrl = new URL("user-settings-assets/clouds.webp", import.meta.url).href;

export type UserSettingsField = "email" | "firstName" | "lastName" | "username";
export type UserSettingsLocale = "en-US" | "pt-BR";

export interface UserSettingsCountry {
...
export interface UserSettingsModalProps<Message> {
  ...
  readonly locale: UserSettingsLocale;
  ...
  readonly onFieldInput: (field: UserSettingsField, value: string) => NoInfer<Message>;
...
const countryDisplayNames: Readonly<Record<UserSettingsLocale, Intl.DisplayNames>> = {
...
const verifiedTick = <Message>(h: HtmlBuilder<Message>): Html =>
  h.svg(
    [
      h.AriaHidden(true),
      h.Class("z-10 size-4 text-utility-blue-500"),
      h.Fill("none"),
      h.ViewBox("0 0 10 10"),
    ],
    [
...
const divider = <Message>(h: HtmlBuilder<Message>): Html =>
  h.svg(
    [h.AriaHidden(true), h.Class("h-[2.5px] w-full max-md:hidden")],
    [
      h.line([
        h.X1("1.2"),
        h.Y1("1.2"),
...
const usernameField = <Message>(
  props: UserSettingsModalProps<Message>,
  h: HtmlBuilder<Message>,
): Html => {
  const inputId = `${props.id}-username`;
  const labels = copy[props.locale];
  return h.div(
    [h.Class("relative flex flex-1 items-center md:w-[277.203125px] md:flex-none")],
    [
      h.div(
        [h.Class("flex w-full flex-col gap-1.5")],
        [
          h.label(
            [h.Class("text-sm font-medium text-text-secondary md:hidden"), h.For(inputId)],
            [`${labels.username} *`],
          ),
          h.div(
            [
...
const settingsForm = <Message>(
  ...
  return h.form(
    [
      h.Class("flex flex-col gap-4 px-4 md:px-6"),
      h.Id(`${props.id}-form`),
    ...
    [
      divider(h),
      h.div(
        [h.Class("flex flex-col gap-4 md:flex-row md:gap-8")],
        [
          h.label(
            [
              h.Class(
                "w-full max-w-50 shrink-0 text-sm font-medium text-text-secondary max-md:hidden",
              ),
              h.For("input-firstname"),
            ],
            [labels.name],
          ),
          h.div(
            [h.Class("flex flex-1 items-end gap-4 md:w-[277.203125px] md:flex-none")],
            [
              h.div(
                [h.Class("min-w-0 flex-1 md:[&>div>span:first-child]:hidden")],
                [
                  input(
                    {
                      isRequired: true,
      ...
      divider(h),
      h.div(
        [h.Class("flex flex-col gap-8 md:flex-row")],
        [
          h.label(
            [
              h.Class(
          ...
          h.div(
            [
              h.Class(
                "flex flex-1 flex-col gap-2 md:w-[277.203125px] md:flex-none md:[&>div>span:first-child]:hidden",
              ),
            ],
            [
              input(
      ...
      divider(h),
      h.div(
        [h.Class("flex flex-col gap-4 max-md:hidden md:flex-row md:gap-8")],
        [
          h.label(
            [
              h.Class("w-full max-w-50 shrink-0 text-sm font-medium text-text-secondary"),
              h.For("select-country"),
            ],
            [labels.country],
          ),
          h.div(
            [h.Class("flex-1 [&>div>label:first-child]:hidden")],
            [
              select(
                {
                  hint: labels.hint,
...
export const userSettingsModal = <Message>(
  props: UserSettingsModalProps<Message>,
  h: HtmlBuilder<Message>,
): Html => {
  const labels = copy[props.locale];
    ...
    props.isOpen
      ? [
          h.div(
            [
              h.Class(
                "fixed inset-0 z-50 flex min-h-dvh w-full items-end justify-center bg-overlay/70 px-4 pt-(--modal-pt) pb-(--modal-pb) [--modal-pb:clamp(16px,8vh,64px)] [--modal-pt:16px] outline-hidden backdrop-blur-[6px] sm:items-center sm:justify-center sm:px-8 sm:[--modal-pb:32px] sm:[--modal-pt:32px]",
              ),
            ...
            [
              ...
              h.dialog(
                [
                  ...
                  h.Attribute("dir", "ltr"),
                  h.Class(
                    "fixed inset-0 mx-auto mt-auto mb-[clamp(16px,8vh,64px)] max-h-[calc(100dvh-var(--modal-pt)-var(--modal-pb))] w-[calc(100%-32px)] overflow-y-auto rounded-xl border-0 bg-bg-primary p-0 align-middle shadow-xl outline-hidden sm:m-auto sm:w-[557.203125px] sm:max-w-160 sm:rounded-2xl",
                  ),
                  h.Id(props.id),
                  h.Lang(props.locale),
                ...
                [
                  h.button(
                    [
                      ...
                      h.Autofocus(true),
                      h.Class(
                        "absolute top-3 right-3 z-20 flex size-9 cursor-pointer items-center justify-center rounded-lg p-2 text-fg-white/70 outline-focus-ring transition duration-100 ease-linear hover:bg-white/20 hover:text-fg-white focus:outline-hidden focus-visible:outline-2 focus-visible:outline-offset-2 sm:top-4 sm:right-4",
                      ),
                      h.DataAttribute("user-settings-close", ""),
                  ...
                  h.div(
                    ...
                    [
                      h.img([
                        h.AriaHidden(true),
                        h.Alt(""),
                        h.Class("h-26 w-full rounded-xl object-cover md:h-32"),
                        h.Src(cloudsUrl),
                      ]),
                    ],
                  ),
                  h.div(
                    ...
                    [
                      ...
                      h.div(
                        [h.Class("flex flex-col items-start gap-4 md:flex-row md:gap-8")],
                        [
                          h.div(
                            ...
                            [
                              h.div(
                                ...
                                [
                                  h.p(
                                    [h.Class("truncate text-lg font-semibold text-text-primary")],
                                    [profileName],
                                  ),
                                  verifiedTick(h),
                                ],
                          ...
                          h.dl(
                            ...
                            [
                              ...
                              h.hr([
                                h.Class("h-11 w-px rounded-full border-none bg-border-primary"),
                              ]),
