/* oxlint-disable effect/noReturnInArrow, effect/noSpread, effect/noTernary -- The controlled native dialog and checkbox list preserve the authenticated modal anatomy. */
import type { Html, HtmlBuilder } from "foldkit/html";

import { button } from "../base/button.ts";
import { checkbox } from "../base/controls.ts";

export type CheckboxesModalChannel = "facebook" | "medium" | "twitter";

export interface CheckboxesModalProps<Message> {
  readonly id: string;
  readonly isOpen: boolean;
  readonly messageForToggle: (channel: CheckboxesModalChannel) => NoInfer<Message>;
  readonly onCancel: NoInfer<Message>;
  readonly onConfirm: NoInfer<Message>;
  readonly onDismiss: NoInfer<Message>;
  readonly selectedChannels: readonly CheckboxesModalChannel[];
}
...
export const checkboxesModal = <Message>(
  props: CheckboxesModalProps<Message>,
              ...
              h.dialog(
                ...
                [
                  ...
                  h.footer(
                    [
                      h.Class(
                        "z-1 flex flex-1 flex-col-reverse gap-3 p-4 pt-6 *:grow sm:grid sm:grid-cols-2 sm:px-6 sm:pt-8 sm:pb-6",
                      ),
                    ],
                    [
                      button(
