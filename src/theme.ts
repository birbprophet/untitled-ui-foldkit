 ...
 *
 * Renderers never hardcode brand hex values. They read semantic Tailwind roles
 * (`bg-bg-brand-solid`, `text-text-brand-secondary`, `outline-focus-ring`, …)
 * that `theme.css` resolves through CSS custom properties. Consumers inject brand
 * colour by defining the eleven `--brand-*` ramp steps before importing
 * `ui/theme.css`.
 */
...
/** Eleven-step brand ramp. The only colour injection surface renderers depend on. */
export type BrandRamp = Record<BrandRampName, string>;

/**
 * Untitled UI v8 default brand ramp (purple), from `untitledui@0.1.64`
